...
const ListBookingRenewal: React.FC<{
  isLoading: boolean;
  isSuccess: boolean;
  isError: boolean;
  data: any;
  ...
  >;
  handleBookingRenewalStatus: (id: string, status: "approved" | "rejected") => void;
}> = ({
  isLoading,
  isSuccess,
  isError,
  ...
  const getColumns = () => [
    // { key: "id", label: t("bookingRenewal.table.requestId"), sortable: true, accessor: (item: any) => item?.id },
    {
      key: "id",
    ...
    {
      key: "name",
      label: t("bookingRenewal.table.customerName"),
      sortable: true,
    ...
    {
      key: "distance",
      label: t("bookingRenewal.table.locationDistance"),
      sortable: true,
      render: (row: any) => {
        return (
          <>
    ...
    {
      key: "update-time",
      label: t("bookingRenewal.table.paymentMethod"),
      sortable: true,
      accessor: (row: any) => row?.booking?.payment_method ?? "",
    },
    { key: "days", label: t("bookingRenewal.table.renewalDuration"), sortable: true },
    {
      key: "rental-duration",
      label: t("bookingRenewal.table.initialRentalDuration"),
      sortable: true,
      accessor: (row: any) => row?.booking?.days ?? "",
    },
    {
      key: "accept-reject",
      label: t("bookingRenewal.table.acceptReject"),
      sortable: false,
      render: (row: any) => {
        return (
          <>
            {row?.status === "approved" && (
              <>
                <Chip
                  label={t("bookingRenewal.table.approved")}
                  color="primary"
                  variant="outlined"
                  sx={{ borderRadius: 1 }}
                />
            ...
            {row?.status === "rejected" && (
              <>
                <Chip
                  label={t("bookingRenewal.table.rejected")}
                  color="error"
                  variant="outlined"
                  sx={{ borderRadius: 1 }}
                />
            ...
            {row?.status !== "approved" && row?.status !== "rejected" && (
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => {
                    handleBookingRenewalStatus(row?.id, "approved");
                  }}
                >
                  {t("bookingRenewal.table.accept")}
                </Button>
                ...
                <Button
                  ...
                  onClick={() => {
                    handleBookingRenewalStatus(row?.id, "rejected");
                  }}
                >
                  {t("bookingRenewal.table.reject")}
                </Button>
              </div>
            )}
          </>
    ...
    {
      key: "actions",
      label: t("bookingRenewal.table.actions"),
      sortable: false,
          ...
          <div className="relative">
            ...
            <TableMenu
              ...
              menuItems={[
                {
                  icon: EyeIcon,
                  label: t("bookingRenewal.table.viewAction"),
                  onClick: () => {
...
export default ListBookingRenewal;
