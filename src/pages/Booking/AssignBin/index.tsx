...
const AssignBin = () => {
  ...
  const removeItem = (binId, itemIndex) => {
    setBins((prevBins) =>
      prevBins.map((bin) =>
        bin.id === binId ? { ...bin, items: bin.items.filter((_, index) => index !== itemIndex) } : bin
      )
    );
  };
    ...
    setBins(
      bins.map((bin) =>
        bin.id === binId
          ? {
              ...bin,
              items: [...bin.items, `ST${bin.items.length + 1}`],
              isExpanded: true,
            }
          : bin
      )
        ...
        <div key={bin.id} className="mb-4 last:mb-0 rounded-lg overflow-hidden">
          ...
          <div className="flex gap-1 items-center justify-between rounded-t-lg px-3 py-3 bg-[#F7F7F7]">
            <div className="flex justify-start md:gap-2 gap-1 items-center">
              <h4 className="md:text-base/tight text-sm/tight font-medium text-green-700">{bin.name}</h4>
              <p className="text-sm text-gray-500 font-medium">(Req. Qty: {bin.reqQty})</p>
            </div>
            <button
              type="button"
          ...
          {/* Expanded Content */}
          {bin.isExpanded && bin.items.length > 0 && (
            <div className="flex justify-between items-center border-gray-200 border-t-0 border rounded-b-lg bg-white">
              <div className="flex flex-wrap 2xl:gap-4 gap-2 2xl:gap-y-2 max-h-[104px] overflow-auto sidebar-scroll p-3">
                {bin.items.map((item, index) => (
                  ...
                  >
                    <span className="md:text-sm text-xs font-medium text-gray-600">{item}</span>
                    <button
                      type="button"
