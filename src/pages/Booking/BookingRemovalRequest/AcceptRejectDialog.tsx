...
const AcceptCancelDialog: React.FC<{
  ...
  bookingRemovalRequestId: string;
  bookingDetailStatus: "approved" | "rejected";
}> = ({ openDialog, setOpenDialog, bookingRemovalRequestId, bookingDetailStatus }) => {
  const { t } = useTranslation();
  const theme = useTheme();
  const queryClient = useQueryClient();
  const acceptCancelBookingRemovalRequestMutation = useMutation({
  ...
  const { values, errors, handleChange, handleBlur, touched, resetForm, handleSubmit } = useFormik({
    ...
    onSubmit: (values) => {
      acceptCancelBookingRemovalRequestMutation?.mutate({
        id: bookingRemovalRequestId,
      ...
      >
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1, mt: 1, ml: 2 }}>
          ...
          >
            ...
            {bookingDetailStatus === "approved" ? t("common.accept") : t("common.reject")}{" "}
            {t("bookingDetails.thisRemovalRequest")}?
          </DialogTitle>
        </Stack>
        ...
        <DialogActions sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button
            ...
            loading={acceptCancelBookingRemovalRequestMutation?.isPending}
          >
