...
const AcceptCancelDialog: React.FC<AcceptCancelDialogProps> = ({
  openDialog,
  setOpenDialog,
  bookingGracePeriodId,
  bookingDetailStatus,
}) => {
  const { t } = useTranslation();
  const theme = useTheme();
  const queryClient = useQueryClient();
  const acceptCancelGracePeriodMutation = useMutation({
  ...
  const { values, errors, handleChange, handleBlur, touched, resetForm, handleSubmit } = useFormik({
    ...
    validationSchema: object().shape({
      reason: bookingDetailStatus === "rejected"
      ? string().trim().required(t("bookingDetails.pleaseEnterRejectionReason"))
      : string().notRequired(),
    }),
    onSubmit: (values) => {
      acceptCancelGracePeriodMutation?.mutate({
        id: bookingGracePeriodId,
      ...
      >
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1, mt: 1, ml: 2 }}>
          ...
          >
            ...
            {t("bookingDetails.thisGracePeriod")}?
          </DialogTitle>
        </Stack>
        ...
        <DialogActions sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button
            ...
            loading={acceptCancelGracePeriodMutation?.isPending}
          >
