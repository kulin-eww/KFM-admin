...
import { assignDriverAPI, updateAssignDriverAPI } from "../../../api/booking";
...
const AssignBinDialog: React.FC<{
  ...
  const handleAssign = useMutation({
    mutationFn: bookingDetailData?.driverWiseContainers?.length > 0 ? updateAssignDriverAPI : assignDriverAPI,
    onSuccess: (res) => {
      ...
      >
        ...
        <DialogActions>
          <Button
            ...
            disabled={selectedBins.length !== bookingDetailData?.total_quantity}
            onClick={() => {
              handleAssign.mutate({ id: id, assign: { assign: selectedBins } });
              setShowAssignBinDialog(false);
            }}
          >
