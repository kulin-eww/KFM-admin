...
const PaymentMethod: React.FC<{ replacementRequestDetailData: any }> = ({ replacementRequestDetailData }) => {
      ...
      <div className="rounded-xl border border-[#EDEDED] md:p-4 p-3">
        <div className="flex justify-between gap-3 items-start pb-3 mb-3 border-b border-[#F7F7F7]">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {replacementRequestDetailData?.replace_payment_status === PAYMENT_STATUS.FAILED && (
              <span className="text-md font-semibold text-red-500">{t("bookingDetails.failed")}</span>
            )}
            {replacementRequestDetailData?.replace_payment_status === PAYMENT_STATUS.PENDING && (
              <span className="text-md font-semibold text-[#FFA500]">{t("bookingDetails.pending")}</span>
            )}
          </div>
          <span className="text-base/tight font-semibold text-primary flex-shrink-0">{t("bookingDetails.online")}</span>
        </div>
        ...
        <div className="flex justify-between items-start gap-2 pb-3 mb-3 border-b border-[#F7F7F7]">
          <div>
            ...
            <p className="text-xs/tight font-medium text-[#676767]">{t("bookingDetails.totalPayableAmount")}</p>
          </div>
          <div className="text-end">
            <p className="text-sm/tight font-semibold text-primary flex gap-1">
              ...
              {replacementRequestDetailData?.replacement_request_total ?? 0}
            </p>
          </div>
        </div>
      </div>
