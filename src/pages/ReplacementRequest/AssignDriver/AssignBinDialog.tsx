...
import { assignDriverForReplacementAPI } from "../../../api/replacementRequest";
...
const AssignBinDialog: React.FC<{
  ...
  const handleAssign = useMutation({
    mutationFn: assignDriverForReplacementAPI,
    onSuccess: (res) => {
      // queryClient.invalidateQueries({ queryKey: ["availableDrivers"] });
      // queryClient.invalidateQueries({ queryKey: ["availableBins"] });
      queryClient.invalidateQueries({ queryKey: ["detailsReplacementRequest", id] });
    ...
    setSelectedBins((prev) => {
      if (checked) {
        return [
          ...
          {
            driver_id: driverId,
            booking_replacement_request_id: bookingDetailId,
        ...
        return prev.filter(
          (item) =>
            !(
              ...
              item.booking_replacement_request_id === bookingDetailId
            )
        );
      }
    });
  ...
  return (
    <>
      <Dialog sx={{ "& .MuiDialog-paper": { maxHeight: 700, minWidth: 400 } }} maxWidth="lg" open={showAssignBinDialog}>
            ...
            {isSuccess && !isLoading && (
              <>
                {selectedDrivers.length === 0 ? (
                  <div className="text-red-600 font-medium mt-4">Please select at least one driver</div>
                ) : (
                            ...
                            >
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  handleBinToggle(
                                    driver.id,
                                    bin.booking_replacement_request_id,
                                    bin.id,
                                    bin.container_code,
                                    e.target.checked
                                  )
        ...
        <DialogActions>
          <Button
            ...
            disabled={selectedBins.length !== replacementRequestDetailData?.totalContainerForReplaceMent}
            onClick={() => {
