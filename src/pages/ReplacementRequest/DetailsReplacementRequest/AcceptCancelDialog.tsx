...
import { acceptRejectReplacementRequestAPI } from "../../../api/replacementRequest";
...
interface AcceptCancelDialogProps {
  ...
  bookingId: string;
  bookingDetailStatus: "approved" | "rejected" | "cancelled" | "";
}
...
const AcceptCancelDialog: React.FC<AcceptCancelDialogProps> = ({
  ...
  const acceptCancelReplacementRequestMutation = useMutation({
    mutationFn: acceptRejectReplacementRequestAPI,
    onSuccess: (res: any) => {
      useToast(res.message);
      queryClient.invalidateQueries({ queryKey: ["detailsReplacementRequest"] });
      setOpenDialog(false);
      resetForm();
    },
    onError: (error) => {
  ...
  const { values, errors, handleChange, handleBlur, touched, resetForm, handleSubmit } = useFormik({
    ...
    onSubmit: (values) => {
      acceptCancelReplacementRequestMutation?.mutate({
        id: bookingId,
      ...
      >
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1, mt: 1, ml: 2 }}>
          ...
          >
            {bookingDetailStatus === "cancelled"
              ...
              : `Are you sure you want to ${bookingDetailStatus === "approved" ? "accept" : "reject"} this booking?`}
          </DialogTitle>
        </Stack>
        ...
        <DialogActions sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button
            ...
            loading={acceptCancelReplacementRequestMutation?.isPending}
          >
